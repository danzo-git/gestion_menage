@extends('layout.app')
<div class="container">
    <h2>vous aimez l'application creer par: <a href="{{route('pages.anim')}}">denzel</a></h2>
</div>
<article class="leaderboard">
    <header>
      ...
      <h1 class="leaderboard__title"><span class="leaderboard__title--top">Famille Kouame</span><span class="leaderboard__title--bottom">Leaderboard</span></h1>
    </header>



    <main class="leaderboard__profiles mb-2">
        @foreach ($taches as $tache)
     <a href="{{route('taches.show',$tache->id)}}"> <article class="leaderboard__profile">
        <img src="{{asset('image/'.$tache->image)}}" alt="Mark Zuckerberg" class="leaderboard__picture">
        <span class="leaderboard__name">{{$tache->nom_tache}}</span>
       <h5><span class="bg-success"> {{$tache->proprietaire_tache}}<span></span></span></h5>
      </article>
    </a>
@endforeach
      {{$taches->links()}}
